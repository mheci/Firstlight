...
- `files/system/` — shipped files:
  ...
  - `etc/greenboot/greenboot.conf` (3 boot attempts, repo/update-platform DNS checks disabled, 1s watchdog grace), `etc/dnf/dnf.conf` (`install_weak_deps=False`), `etc/systemd/{system,user}.conf.d/00-timeout.conf` (15s stop timeout), `etc/pki/firstlight-mok/` (cert.pem, db.der).
  - `usr/lib/udev/rules.d/` (`60-schedulers.rules` kyber/bfq, `80-nvidia-pm.rules` — same contents as Bazzite), `usr/lib/sysctl.d/`, `usr/lib/modprobe.d/nvidia-power.conf`, `usr/lib/systemd/system/` (`dev-hugepages1G.mount`, `fstrim.service.d/01-quiet-unsupported.conf`, `mcelog.service.d/override.conf`), `usr/lib/dracut/dracut.conf.d/80-vfio.conf` (vfio iommu drivers), `usr/bin/dlss-swapper` (CachyOS DLSS wrapper).
